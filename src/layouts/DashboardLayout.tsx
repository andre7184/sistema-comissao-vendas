...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    <div className="flex h-screen bg-surface dark:bg-slate-900 text-gray-800 dark:text-gray-100 font-sans overflow-hidden transition-colors duration-300">
      ...
      {/* --- SIDEBAR --- */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 shadow-sm transform transition-transform duration-300 ease-in-out md:static md:translate-x-0 
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Logo Area */}
        <div className="flex items-center justify-between h-16 px-6 border-b border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800">
          <div className="flex items-center gap-2 text-brand-700 dark:text-brand-400">
            <div className="p-1.5 bg-brand-50 dark:bg-slate-700 rounded-lg">
                <Building size={24} className="text-brand-600 dark:text-brand-400" />
            </div>
            <span className="text-lg font-bold tracking-tight text-gray-900 dark:text-white">Gestão<span className="text-brand-600 dark:text-brand-400">Pro</span></span>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Header */}
        <header className="bg-white dark:bg-slate-800 h-16 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center px-4 sm:px-6 lg:px-8 shadow-sm relative z-30">
          <button onClick={() => setSidebarOpen(true)} className="md:hidden text-gray-500 dark:text-gray-400 p-1 rounded-md">
            <Menu size={24} />
          </button>
          
          <h1 className="text-lg font-semibold text-gray-700 dark:text-gray-200 invisible md:visible">
            Painel de Controle
          </h1>
